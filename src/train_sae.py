...
model = HookedTransformer.from_pretrained(
    "google/gemma-2b-it", dtype='float16'
)  # This will wrap huggingface models and has lots of nice utilities.

embed_layer = model.to('cpu').W_U.detach().clone()
...
DATASET="hwatkins/tokenized_wiki_medical"
...
cfg = LanguageModelSAERunnerConfig(
    ...
    model_name=MODEL,  # our model (more options here: https://neelnanda-io.github.io/TransformerLens/generated/model_properties_table.html)
    hook_name="blocks.0.hook_mlp_out",  # A valid hook point (see more details here: https://neelnanda-io.github.io/TransformerLens/generated/demos/Main_Demo.html#Hook-Points)
    hook_layer=0,  # Only one layer in the model.
    d_in=2048,  # the width of the mlp output.
    dataset_path=DATASET,  # this is a tokenized language dataset on Huggingface for the Tiny Stories corpus.
...
sae_layer = sparse_autoencoder.to('cpu').W_dec.detach().clone()
del sparse_autoencoder

# Let's start by getting the top 10 logits for each feature
projection_onto_unembed = sae_layer @ embed_layer#model.W_U
